...
with col1:
    ...
    with col12:
        regular_network_architecture_selection = st.selectbox('Network Architecture', ('ResNet 110, no skip connections', 'ResNet56, no skip connections', 'VGG16'), key=12)
    ...
    with col16:
        regular_learning_rate_selection = st.selectbox('Learning Rate', ('0.05', '0.1', '0.001'), key=16)
    col17, col18 = st.columns(2)
    with col17:
        regular_momentum_selection = st.selectbox('Momentum', ('0.9',), key=17)
    with col18:
        regular_weight_decay_selection = st.selectbox('Weight Decay', ('0','0.0005'), key=18)
...
with col2:
    ...
    with col27:
        lm_learning_rate_selection = st.selectbox('Learning Rate', ('0.05','0.1', '0.001'), key=27)
    with col28:
        lm_momentum_selection = st.selectbox('Momentum', ('0.9',), key=28)
    col29, col210, col211 = st.columns(3)
    with col29:
        lm_weight_decay_selection = st.selectbox('Weight Decay', ('0','0.0005'), key=29)
    with col210:
        lm_C_selection = st.selectbox('C for LM', ('2','1', 'adaptive'), key=210)
    with col211:
        lm_f_selection = st.selectbox('f', ('f(x) = x', 'f(x) = x^2', 'f(x) = arctan(x^2)'), key=211)
